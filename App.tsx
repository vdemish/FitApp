  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark flex justify-center selection:bg-primary selection:text-white overflow-x-hidden">
      ...
      <div className="w-full max-w-lg flex flex-col min-h-screen relative">
        {/* Header Navigation with Bolt/Timer */}
        <header className="flex-none px-4 pt-8 pb-4 sticky top-0 z-40 bg-background-light/50 dark:bg-background-dark/50 backdrop-blur-xl">
          <div className="flex items-center justify-between">
            ...
            <button className="w-10 h-10 rounded-2xl liquid-glass border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-500 dark:text-white/60">
        ...
        {/* Floating Action Bar (Conditional for Workout) */}
        {activeTab === 'workout' && (
          <div className="fixed bottom-32 left-0 right-0 z-50 px-4 pointer-events-none">
            <div className="max-w-lg mx-auto pointer-events-auto">
              <div className="p-4 pt-2 pb-2 liquid-glass border border-slate-200/50 dark:border-white/10 rounded-3xl flex gap-4 shadow-2xl">
                <button className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-white/5 border border-slate-200 dark:border-white/10 flex items-center justify-center text-slate-400 dark:text-white/70 active:bg-primary/20 transition-colors">
...
const NavItem: React.FC<{
  ...
  <button 
    ...
    className={`flex flex-col items-center gap-1 transition-all duration-300 ${
      active ? 'text-primary scale-110' : 'text-slate-400 dark:text-white/30 hover:text-slate-600 dark:hover:text-white/60'
    }`}
  >
